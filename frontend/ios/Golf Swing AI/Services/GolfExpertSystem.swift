...
class GolfExpertSystem: ObservableObject {
    ...
    @Published var unknownQuestionsCount = 0

    private var conversationHistory: [Message] = []
    private let maxHistoryLength = 10

    // Feedback & Learning
    private let feedbackManager = CaddieFeedbackManager.shared
    ...
    private func analyzeAndRespond(to question: String) async -> String {
            ...
            for pattern in category.patterns {
                if lowercased.contains(pattern) {
                    ...
                    if let response = category.responses.first(where: { response in
                        ...
                        feedbackManager.logSuccessfulMatch(question: question, pattern: pattern, category: category.category)
        ...
        // Check user-added custom responses first
        if let customResponse = feedbackManager.getCustomResponse(for: lowercased) {
            return customResponse
        }

        // Log unknown question for future improvement
        feedbackManager.logUnknownQuestion(question)
        unknownQuestionsCount = feedbackManager.getUnknownQuestionsCount()

        // Honest fallback - admit we don't have a specific answer
        return """
I don't have a specific answer for "\(question)" in my knowledge base yet.

**Here's what I can help with**:
• Golf rules (penalties, drops, obstructions)
• Swing fixes (slice, hook, fat/thin shots)
• Equipment (clubs, loft, shaft flex)
• Course strategy & mental game
• Short game (putting, chipping, bunkers)

**Can you rephrase your question?** For example:
• "How do I fix my slice?"
• "What's the rule for a cart path?"
• "What loft should my wedges be?"

Your question has been logged - I'm always learning!
"""
    }

    // MARK: - Fast Pattern-Match Answer (No side effects, no history)

    /// Returns a confident answer if the expert system's patterns match the query.
    /// Returns nil if no pattern matched — the caller should fall through to the LLM.
    /// Unlike sendMessage(), this does NOT add to conversation history.
    func tryAnswer(_ query: String) -> String? {
        ...
        // Check any user-submitted custom responses
        if let customResponse = feedbackManager.getCustomResponse(for: lowercased) {
            return customResponse
        }
    ...
    func rateResponse(question: String, response: String, helpful: Bool) {
        feedbackManager.recordRating(question: question, response: response, helpful: helpful)
    }
    ...
    func submitCorrection(question: String, wrongResponse: String, correctAnswer: String) {
        feedbackManager.submitCorrection(question: question, wrongResponse: wrongResponse, correctAnswer: correctAnswer)
    }
...
struct GolfRulesResponses {
    static let all: [GolfResponse] = [
        GolfResponse(
            triggers: ["sprinkler", "sprinkler head", "irrigation", "drain", "drainage"],
            answer: """
**Ball on Sprinkler Head / Irrigation Box** (Rule 16.1):

Sprinkler heads are **immovable obstructions** - you get **free relief**!

**Procedure**:
• Find nearest point of complete relief (no interference with stance/swing)
• Drop within **1 club-length**, no closer to the hole
• Must stay in same area (fairway stays fairway, rough stays rough)
• **No penalty stroke**

**When you get relief**:
• Ball is on or touching the sprinkler
• Sprinkler interferes with your stance
• Sprinkler interferes with your swing

**Pro tip**: You don't get relief just because it's on your line of play (unless local rule says otherwise).
"""
        ),
