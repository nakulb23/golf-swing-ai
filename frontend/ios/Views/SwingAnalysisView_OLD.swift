...
// Temporary import to resolve compilation issues
// Remove this once target membership is properly configured
#if DEBUG
// This import provides temporary type definitions while resolving build issues
#endif
...
struct VideoSelectionView: View {
    ...
    @StateObject private var historyManager = AnalysisHistoryManager.shared
                        ...
                        Button(action: onShowHistory) {
                            HStack(spacing: 12) {
                                Image(systemName: "clock.arrow.circlepath")
                                    .font(.title2)
                                Text("Analysis History (\(historyManager.analysisHistory.count))")
                                    .font(.headline)
                                    .fontWeight(.semibold)
                            }
                            .foregroundColor(.white.opacity(0.8))
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    ...
                    // Save analysis to history with video data
                    historyManager.addAnalysis(result, videoData: videoData)
                }
            } catch {
                await MainActor.run {
                    isAnalyzing = false
            ...
            ZStack {
                ...
                if let analysisResult = analysisResult, let videoData = videoData {
                    ...
                    VideoSelectionView(
                        ...
                        analysisResult: $analysisResult,
                        onShowHistory: { showingAnalysisHistory = true }
                    )
                }
            }
        }
...
struct SwingVideoPreview: View {
    ...
    var body: some View {
        ZStack {
            ...
            .animation(Animation.easeInOut(duration: 0.3), value: isPlaying)
        ...
        ZStack {
            ...
            ForEach(getPlaneDeviationPoints(), id: \.frame) { point in
                Circle()
                    ...
                    .animation(Animation.easeInOut(duration: 0.3), value: currentTime)
                }
            }
        }
        ...
        ZStack {
            ...
            if let currentPosition = getCurrentClubPosition() {
                Circle()
                    ...
                    .animation(Animation.easeInOut(duration: 0.1), value: currentTime)
                }
            }
        }
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                Circle()
                    ...
                    .animation(Animation.easeInOut(duration: 0.1), value: ballPosition)
...
struct VideoFirstAnalysisView: View {
    ...
    @StateObject private var localAnalyzer = LocalSwingAnalyzer()
    
    // Fixed default - no longer user-configurable
    private let isRightHanded = true
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            GeometryReader { geometry in
                ...
                // Centered Play Button - Shows when paused, hides when playing
                if !isPlaying {
                    VStack {
                        Spacer()
                        HStack {
        ...
        .sheet(isPresented: $showFeedbackPrompt) {
            // TEMPORARILY COMMENTED - Fix target membership in Xcode
            EmptyView()
            // FULL FEEDBACK BLOCK COMMENTED OUT - Fix target membership in Xcode
            /*
            FeedbackPromptView(
                prediction: analysisResult.predicted_label,
                confidence: analysisResult.confidence
                ...
                if let feedback = feedback {
                    // Get real features from the local analyzer's stored prediction
                    let features = localAnalyzer.getCurrentPredictionForFeedback() ?? []
                    
                    // Store feedback with BOTH local and centralized collectors
                    Task { @MainActor in
                        let metadata = SwingMetadata(
                            videoDuration: videoDuration,
                            deviceModel: UIDevice.current.model,
                            appVersion: "1.0.0",
                            analysisDate: Date(),
                            userSkillLevel: nil as String?,
                            clubType: nil as String?,
                            practiceOrRound: nil as String?
                        )
                        
                        // Send to centralized system (benefits all users)
                        CentralizedModelImprovement.shared.collectPredictionData(
                            features: features, // ✅ Real physics features from pose analysis
                            modelPrediction: analysisResult.predicted_label,
                            modelConfidence: analysisResult.confidence,
                            userFeedback: feedback, // ✅ User corrections and ratings
                            swingMetadata: metadata,
                            isFromLocalModel: true
                        )
                        
                        // Also store locally (backwards compatibility)
    ...
    func checkForFeedbackPrompt() {
        // Check if we should prompt for feedback based on confidence
        if analysisResult.confidence < 0.8 || (analysisResult.confidence < 0.9 && Bool.random()) {
...
struct AnalysisHistoryView: View {
    ...
    // @StateObject private var exportManager = AnalysisExportManager.shared // TEMPORARILY COMMENTED
    @Environment(\.dismiss) private var dismiss
    @State private var selectedEntry: AnalysisHistoryEntry?
    @State private var showingExportOptions = false
    @State private var showingDeleteAlert = false
    @State private var entryToDelete: AnalysisHistoryEntry?
        ...
        ZStack {
            ...
            if historyManager.analysisHistory.isEmpty {
                ...
                VStack(spacing: 24) {
                    ...
                    Text("Your recent swing analyses will appear here")
                        .font(.body)
                        .foregroundColor(.white.opacity(0.7))
                        .multilineTextAlignment(.center)
                    }
                ...
                ScrollView {
                    LazyVStack(spacing: 16) {
                        ForEach(historyManager.analysisHistory) { entry in
                            AnalysisHistoryCard(
                                entry: entry,
                                ...
                                onSavePermanently: {
                                    historyManager.markAsPermanentlySaved(id: entry.id)
                                    }
                            )
                            }
                        }
    ...
    .sheet(item: $selectedEntry) { entry in
        AnalysisDetailView(entry: entry)
        }
            ...
            buttons: [
                .default(Text("Export as PDF")) {
                    if let entry = selectedEntry {
                        exportAnalysisAsPDF(entry)
                        }
                },
                .default(Text("Export Video with Stats")) {
                    if let entry = selectedEntry {
                        exportVideoWithOverlay(entry)
                        }
                },
                .default(Text("Share Analysis")) {
                    if let entry = selectedEntry {
                        shareAnalysis(entry)
                        }
                },
    ...
    .alert("Delete Analysis", isPresented: $showingDeleteAlert) {
        Button("Delete", role: .destructive) {
            if let entry = entryToDelete {
                historyManager.removeAnalysis(withId: entry.id)
                }
            }
        Button("Cancel", role: .cancel) { }
    } message: {
    ...
    private func exportAnalysisAsPDF(_ entry: AnalysisHistoryEntry) {
    if let pdfURL = exportManager.exportAnalysisAsPDF(entry) {
        shareItems = [pdfURL]
        showingShareSheet = true
        }
    }
    
    private func shareAnalysis(_ entry: AnalysisHistoryEntry) {
    shareItems = exportManager.shareAnalysis(entry)
    showingShareSheet = true
    }
    
    private func exportVideoWithOverlay(_ entry: AnalysisHistoryEntry) {
    Task {
        if let videoURL = await exportManager.exportVideoWithOverlay(entry) {
            await MainActor.run {
                shareItems = [videoURL]
                showingShareSheet = true
                }
...
struct AnalysisHistoryCard: View {
    ...
    let onDelete: () -> Void
    let onExport: () -> Void
    let onSavePermanently: () -> Void
    
    var body: some View {
    Button(action: onTap) {
        HStack(spacing: 16) {
            // Video thumbnail or analysis indicator
            VStack {
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack {
                    Text(entry.analysisResult.predicted_label.replacingOccurrences(of: "_", with: " ").capitalized)
                        .font(.headline)
                        .foregroundColor(.white)
                ...
                HStack {
                    Text("Confidence: \(Int(entry.analysisResult.confidence * 100))%")
                        .font(.subheadline)
                        .foregroundColor(.white.opacity(0.8))
                    ...
                    // Action buttons
                    HStack(spacing: 8) {
                        if !entry.isPermanentlySaved {
                            Button(action: onSavePermanently) {
    ...
    switch entry.analysisResult.predicted_label {
    case "good_swing":
        return .green
    case "too_steep":
        return .orange
    ...
    NavigationView {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                    ...
                    HStack {
                        VStack(alignment: .leading) {
                            Text("Confidence")
                                .font(.caption)
                                .foregroundColor(.white.opacity(0.7))
                            Text("\(Int(entry.analysisResult.confidence * 100))%")
                                .font(.title2)
                                .fontWeight(.semibold)
                                .foregroundColor(.white)
                            }
                ...
                // Physics insights
                if !entry.analysisResult.physics_insights.plane_analysis.isEmpty {
                    VStack(alignment: .leading, spacing: 8) {
                        Text("Analysis Notes")
                            .font(.headline)
                            .foregroundColor(.white)
                        
                        Text(entry.analysisResult.physics_insights.plane_analysis)
                            .font(.body)
                            .foregroundColor(.white.opacity(0.8))
                            .padding()
                            .background(Color.white.opacity(0.1))
